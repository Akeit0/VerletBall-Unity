...
namespace ZimGui {
    ...
    public static class IM {
        ...
        public static WObject Add(string name, Func<bool> func) {
            var w=new WObject(name, func);
             WObjects.Add(w);
             return w;
        }
        ...
        public static void Label(Str label, float value) {
            ...
            if (value.TryFormat(remain, out var count,IMStyle.FloatFormat)) {
        ...
        public static void Quad(Rect rect,UiColor color) {
            Mesh.AddRect(rect, color);
        }
        public static void Quad(Color color) {
            if (!Current.TryGetNextRect(out var rect)) return;
            Mesh.AddRect(rect,color);
        }
        ...
        public static bool Slider(Rect rect, Str label, ref float value, float min,
            float max ) {
            var showValue = rect.DivideToLabelAndValue(out var labelRect, out var valueRect);
            Label(labelRect, label);
            var contains = labelRect.ContainsActiveMouse();
            ...
            if (focusState == FocusState.NewFocus) {
                ...
                v = Mathf.Clamp(v, min, max);
                if (v != value) {
                   if(0.2<max-min) value = Mathf.Floor(v * 100) / 100;
                    changed = true;
                }
            }
            if (!showValue) return changed;
            valueRect.TryDivide(valueRect.width * 2 / 3, out var sliderRect, out var inputRect);
             changed |= Slider(sliderRect,ref  value,min,max);
            changed|= InputField<SingleParserFormatter, float>(inputRect, ref value);
            return changed;
            
        }
 public static bool Slider(Rect rect, Str label, ref int value, int min,
     int max ) {
            var showValue = rect.DivideToLabelAndValue(out var labelRect, out var valueRect);
            Label(labelRect, label);
            var contains = labelRect.ContainsActiveMouse();
            ...
            valueRect.TryDivide(valueRect.width * 2 / 3, out var sliderRect, out var inputRect);
             changed |= Slider(sliderRect,ref  value,min,max);
            changed|= InputField<Int32ParserFormatter, int>(inputRect, ref value);
            return changed;
        ...
        public static bool Slider(Str label, ref float value, float min ,
            float max ) {
            if (!Current.TryGetNextRect(IMStyle.SpacedTextHeight, out var rect)) return false;
            return Slider(rect, label, ref value, min, max);
        }
        public static bool Slider(Str label, ref int value, int min ,
            int max ) {
            if (!Current.TryGetNextRect(IMStyle.SpacedTextHeight, out var rect)) return false;
            return Slider(rect, label, ref value, min, max);
        }
        ...
        public static bool Slider(Rect rect, ref float value, float min, float max) {
            ...
            if (focus) {
                ...
                if (currentX == newRectXMax) value = max;
                    ...
                    value = min + (max - min) * ((currentX - newRectXMin) / rectWidth);
                    if(0.2<max-min) value = Mathf.Floor(value * 100) / 100;
                }
        ...
        public static bool Slider( Rect rect,ref int value, int min, int max) {
            var instance = Mesh;
            var currentValue = value = Mathf.Clamp(value, min, max);
